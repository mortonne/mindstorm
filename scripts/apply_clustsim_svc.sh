...
if [ $# -lt 3 ]; then
    ...
    echo
    echo "Usage:   apply_clustsim_res.sh [-o] filepath model maskname"
    echo "Example: apply_clustsim_res.sh mvpa/cat_react_item_rt study_stim b_par"
    echo
    exit 1
fi
...
if [ $(imtest $statdir/zstat_vox_p_tstat1) = 1 -a $(imtest $statdir/stat_thresh) = 0 ]; then
   recalc=true
fi
